...
# helper classes
from classcharts import Activity, AttendanceData, AttendanceMeta, Announcements, Detentions, Homework, Session, Student, Timetable
...
def _get_academicreport(session_id, student_id):
    ...
    headers = {'Content-Type' : 'application/json', 'Authorization': f'Basic {session_id}'}
...
def _get_activity(session_id, student_id, days=30, save_csv=False):
    ...
    if response['success'] == 1:
        ...
        while last_id != activities[-1].id:
            header = ['ID', 'Timestamp', 'Type', 'Polarity', 'Reason', 'Score', 'Lesson Name', 'Teacher', 'Notes']
            activity_data = [header]
            for activity in activities:
                act = [activity.id, activity.timestamp, activity.type, activity.polarity, activity.reason, activity.score, activity.lesson_name, activity.teacher_name, activity.note]
        ...
        if save_csv:
            ...
            with open(csv_file, 'w') as csv_file:
                ...
                for activity in activities:
                    csv_writer.writerow([activity.id, activity.timestamp, activity.type, activity.polarity, activity.reason, activity.score, activity.lesson_name, activity.teacher_name, activity.note])
...
def _get_homework(session_id, student_id, display_type, days, index=None):
    ...
    if response['success'] == 1:
        ...
        print(f"Assignments completed this week: {response['meta']['this_week_completed_count']}")
        print(f"Assignments outstanding this week: {response['meta']['this_week_outstanding_count']}")
...
def _get_timetable(session_id, student_id, date_required=date.today()):
    ...
    if response['success'] == 1:
        ...
        timetable_day_data = []
        period_data = {}
        for date in response['meta']['timetable_dates']:
...
def parse_args(args=None):
    ...
    parser_academicreport = subparsers.add_parser('academicreport', help='get student academic report')
    ...
    parser_customfields = subparsers.add_parser('customfields', help='get student customfields')
